...
export function CardStack<T extends CardStackItem>({
  ...
  // Responsive card width: never exceed container width minus 32px padding
  const containerRef = React.useRef<HTMLDivElement>(null);
  const [containerWidth, setContainerWidth] = React.useState<number>(cardWidth);
  ...
  const effectiveCardWidth = Math.min(cardWidth, containerWidth - 32);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(300, effectiveCardHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
        aria-label="Portfolio card stack. Use arrow keys to navigate."
      >
        <div
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          {activeItem.href ? (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="text-zinc-400 hover:text-accent transition-colors ml-2"
              aria-label="Abrir link do projeto"
            >
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full bg-zinc-900">
      ...
      {/* Active glow border */}
      {active && (
        <div className="pointer-events-none absolute inset-0 rounded-2xl ring-2 ring-accent/60" />
      )}

      {/* content */}
      <div className="relative z-10 flex h-full flex-col justify-end p-5">
        {(item.tag ?? item.niche) ? (
          <span className="inline-block mb-2 px-2.5 py-0.5 rounded-full bg-accent/20 border border-accent/40 text-accent text-[10px] font-semibold uppercase tracking-widest w-fit">
            {item.tag ?? item.niche}
          </span>
        ) : null}
